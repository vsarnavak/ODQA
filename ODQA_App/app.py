...
def finalfun(test_question):
    ...
    if len(answer)!=0:
        ...
        return "Sorry:(... Unable to fetch appropriate results!"
    # return [test_context, answer]
...
sample_questions = ["Sample Questions : ", "செயற்கை நுண்ணறிவு என்றால் என்ன?", "திருக்குறள் என்றால் என்ன?", "பெரியார் எப்போது பிறந்தார்?"]
...
def main():
    ...
    if form.validate_on_submit():
        qn = form.question.data
        answer = finalfun(qn)
        
        return render_template("base.html", form = form, answer = answer[1], context = answer[0], sample_questions = sample_questions, check = True)
    return render_template("base.html", form = form, sample_questions = sample_questions)
